...
3. [Kiến Trúc Tổng Thể Với Model Có Sẵn](#3-kiến-trúc-tổng-thể-với-model-có-sẵn)
...
8. [Model Pretrained & Cơ Chế Lựa Chọn Mô Hình](#8-model-pretrained--cơ-chế-lựa-chọn-mô-hình)
...
## 3. Kiến Trúc Tổng Thể Với Model Có Sẵn

Hệ thống sử dụng model YOLO đã được huấn luyện sẵn trên MS COCO. Vì vậy, luồng chính là **Inference Pipeline**; nhóm không huấn luyện lại model và không cần xây dựng dataset training trong phạm vi demo:
...
```text
               ...
               +--------------+--------------+
               v                             v
            INPUT SOURCES                 MODEL CONFIG
            Image / Video / Webcam        confidence / IoU / device
               |                             |
               +--------------+--------------+
                      v
                  PREPROCESSING
              ...
              Bounding Box + Class + Confidence
                 |                 |
                 v                 v
               Hien thi / Luu       Danh gia FPS
```

### Phân Định Các Thành Phần:
* **Model pretrained:** Trọng số đã học sẵn trên 80 lớp MS COCO; model được tải tự động hoặc đặt trong thư mục `models/`.
* **Inference pipeline:** Nhận input, tiền xử lý, chạy Backbone -> Neck -> Head, sau đó lọc kết quả bằng Confidence và NMS.
* **Output pipeline:** Hiển thị hoặc lưu ảnh/video đã vẽ box; webcam hiển thị trực tiếp.
* **Evaluation:** Đo FPS/latency thực tế. Precision, Recall và mAP chỉ dùng khi có ground truth tương ứng.
...
## 8. Model Pretrained & Cơ Chế Lựa Chọn Mô Hình
...
### 8.2. Model Có Sẵn Và Cách Chọn
Hệ thống dùng model pretrained, không khởi tạo model từ đầu và không train lại. Có thể đổi file trọng số qua giao diện hoặc tham số dòng lệnh:
...
> 1. **Tốc độ phần cứng máy chạy demo:** Chọn `yolov8n.pt` làm mặc định vì nhỏ và phù hợp máy cá nhân. Ultralytics sẽ tự tải model nếu file chưa có trong máy.
...
> 4. **Khả năng chuyển đổi mượt mà:** Người dùng có thể đổi model ngay trên giao diện hoặc truyền `--weights <ten_model>.pt`.
...
│
├── models/                        # Tùy chọn: lưu các file trọng số YOLO (.pt)
